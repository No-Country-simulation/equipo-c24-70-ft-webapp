...
const Register = () => {
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(async () => {
            try {
                const response = await fetch(import.meta.env.VITE_BACKEND_URL + "/validate-password", {
                    method: "POST",
    ...
    return (
        <div className="register flex items-center justify-center min-h-[37rem] relative">
            <img src={LoginBanner} alt="Fondo de login" className="bannerLogin absolute" />
            <img src={LaptopRegister} alt="Laptop Register" className="absolute laptopRegister" />
            <img src={CircleRegister} alt="Circle Register" className="absolute circleRegister" />
            <div className="relative w-full max-w-md rounded-lg bg-white p-6 shadow-md backdrop-blur-md bg-opacity-90 transition-transform duration-300 hover:-translate-y-2 hover:shadow-lg">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="w-full h-2 mt-2 rounded bg-gray-300">
                        <div className={`h-2 ${strengthColor} rounded`} style={{ width: `${(passwordScore + 1) * 20}%` }}></div>
                    </div>
                    {timeToCrack && (
            ...
            <button type="submit" className="btn__primary btn__lime w-full" disabled={!state.password || passwordScore < 3}>
                Regístrate
            </button>
            {passwordScore < 3 && (
                <p className="text-red-500 text-sm mt-1">La contraseña es demasiado débil. Usa una más segura.</p>
            )}
        </form>
            </div >
        </div >
